...
*   **How do I change the database port? ** Modify the `DATABASE_URL ` environment variable, ensuring to change the Port component with the correct values.
*   **What happens if I run `cargo run` and I see ` "schema migration needed` error? ** This typically mean database does not match what exists for this code- version. To correct it, run `./ scripts/ db_migrate ` to ensure everything stays current with your codebase.
*   ** Where does my data get stored if my server restarts?**  Tempo relies on Postgres for data persistency and all the configuration, data & state is backed up to database, thus ensuring your data gets properly backed. 

## Citation

If you're utilizing Tempo for academic or research endeavors, please cite as follows:

```bibtex
@software{tempo,
  author = {The Tempo Developers},
  title = {Tempo - Task Management Engine},
  url = {https://github.com/[repo_url] - replace with project repository link},
  version = {1.0.0},
  year = {2024},
  license = {MIT}
}
...
For feedback, feature requests, or bug reports, contact us via:

*   Email: [devs@tempo.io] - replaced here
*   Discord: [link - optional, add if available ]- replace here
*   GitHub Issues: [project's issue tracker - linked with repo above ]- replaced here
