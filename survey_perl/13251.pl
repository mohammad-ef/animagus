...
detect_compiler();   # Start checking compiler
configure_flags();
system_checks();  # Perform checks

execute_phase ("make - j4 all");

# Execute other phase here as required for the installation
build(); # Calling for an executing the phase. This could also involve testing for build issues

test()
{

 print ' The final diagnostics have completed successfully and are stored within ' ;

 }


print 'Final build automation completed with success. A summary may now be displayed \n';

exit  0; # All checks successful; exit code zero, meaning successful completion; This is standard.