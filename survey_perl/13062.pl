...
sub detect_compiler { # detect what compiler and other tools will be used for the build
    ...
    my $compiler_version = `$compiler --version 2>&1`;
        print "Compiler Version :  " . $compiler_version. "\n";
    return   $ compiler   ;  #  it will store the detected command to the variable to be called and executed. This enables using different compilers on the different architectures of systems to avoid configuration or setup problems	}
...
print  " Running  Initial Environment... \n" ;  ## start printing logs of all processes running during compilation 

 # Call init but dont save return for the purpose of simplicity for demonstration 
 #init_environment ( )    
 print   " Initialization completed  "  ; # indicates to users and other developers, the process to execute the build has ended	 

   print "\n\n System Diagnostics Information \n\n ";  # print a section header to display some information

print "\t System OS details\t".`uname -a` ; ## display details and system details of running system

    
   

exit   0  ;# indicates a proper script end to show no issues occurred, or the process completed successfully and it is available and safe to run other tasks
