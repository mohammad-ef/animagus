...
for my $file(@file s ) {
 ...
 log_ msg("Starting recovery from the last successul build" )}

# Define project
  my ($ PROJECT_NAM  E) = "testbuild_app ";  # Program's main Name
  my ($ PR OJECT  _D IR )  ="proj ";   # The location of build metadata, such as project build logs to keep them organized, version metadata or any other configuration file to be maintained

log msg( "The project name being build  : '$PRO JECT N AME'");

configure ();

buil d ()  or war  n_exit  ( 'Failed during compil e.   ' ) ; #  Check compiler. If failed at build exit the code
t est();
  # Check the status to verify all test passed to proceed
  create package(); # Creates build
install (); # Lastly Install
recover ()  # Start Recovery from Last successful build
cleanup ($PR  OJ EC T   _DI R);   # Clear any files from project after all operations were successful in code execution! 
print "\n  Success ul completion!"  or  say "\n  An e rror occured!";

   exit  ( 0);   # If success the process exits (0)
