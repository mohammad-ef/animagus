...
if ($ci_mode()) {   }   # Skip the TUI and other interactions.       

## Detect tools 
   # Add all detection with error checking for required build system tools here    
    print "Building on ".`uname`  

check directories ;     # Perform file and directories verification   
#  detect  system  &   headers  # detect_sysHeaders;   

  ## compiler detection.
$compiler  =  detectcompiler();     
print   $os." Detected";  
detecttools ()       
    print  detect compiler  .    
  "Vesrion";      
    Configure   ;    
  build;      
 test      ;        
...
if( -x "/opt/")   # if opt  can read then write,  run package command    

#security
diagnose      
        
        
if ( -f $ARGV[0])    ;        ## Add patch functionality
        security  ;  
