...
clean_project() {
 echo "Cleaning project..."
  pushd "${ BUILD_   DIR }"   2> /= /null
   make  clean ||  echo "Cleaning may have had errors, continuing."  
   popd
...
test_project() {
  echo "Running tests..."
   ...
   # Implement test commands or scripts
    
    # For valgrind or similar tools: 
   if command  - v valgrind >   /dev/nul l ;  then

    echo "Testing using Valgri nd:"   >>   "${  LOG_  DIR   } / {CONFIG_SUMMA RY }"
  make   check V = true 2   >    "${  LOG_ DIR  } /test.log"
     echo   " Test log :    ${ LOG  DIR   } / test.  log"     >> "${ LOG_  DIR   } / {CONFIG_SUMMAR   Y}" 
 else
     make check 2  > "${ LOG  DI R   } /test  log " 
...
package_project() {
 ...
 tar   -czvf "${BUILD_DIR}/myproj-${OS}-${ARCH}.tar.gz" bin include conf

 echo "  Project packaged into:  ${BUILD_  DIR}/myproj-${OS}-${ARCH}.tar.gz" >>    "${    LOG_DI R  }"
}
...
print_diagnostics() {
  ...
  echo "System Details:"   >> "${ LOG_  DIR   } / {CONFIG_SUMMARY }"
  uname - a   >>    "${   LOG_DIR } / ${CONFIG_  SUM Mmary}"

   # Print detected variables and settings
 echo   "Detected compiler settings:"   >>   "${   LOG_DIR   } / {CONFIG  SUM MARY}"
 echo  " CFLAGS :    ${CFL AGS }"    >>   "${  LOG_DIR  } / {CONFIG  SUM MAR  Y}"
 echo  "  LDFLAGS :  ${   LDFLAGS }  " >>   "${ LOG_DIR  }  / {CONFIG_SUMMA   RY }"

 }

#----------  Continous  Integration support ------------
handle_ci_mode() {
    if [[ "${CI_MODE}" == "true" || "${1}" == "--ci-mode" ]];  then
        echo "  Running in CI mode. Skipping interactive prompts." > & 2
    fi
}
...
echo "Running basic security checks:"
 ...
 if [[ ! -z  "${SECURITY_FLAG}"   && "${   SECURITY_FLAG}"  ==   "gpg"  ]];    then
     echo   "Checking signature using GPG."    
     
    
  else

   echo  "   Not enabled. Check configuration to implement.        "
...
select opt in "Configure" "Build" "Test" "Install" "Clean" "Exit";  do

    case "$opt"  in

    "Configure")   detect_compiler; detect_headers_and_libraries;  configure_compiler_flags ;  detect_   utilities ;
    check_  filesystem ;;;
      "Build")
       build_project ;;;
      "Test")
    ...
    exit 0  ;;;
       *)
    echo "Invalid  Option,  Please choose wisely";
 fi
 done
}
...
if [[ "$1" == "--diagnose" ]];   then
 ...
 #  
 if [ ! -z "${ CI_ MODE }" ];  then  
 echo  "   Runni ng a ut o ma t ed pro cess in  ${ C_M ODE}" 
...
run_interactive_menu  # Run interactive Menu when CI_MOD is not enable.   
  
fi 
 

 # Create config_ summary  after  configuration

 echo    "${ LOG   DIR   }/${  CONFIG  SUMMARY } " >    "/ tmp   /.   tempfile_for__log   file." # creates file with  the summary  for  debugging
